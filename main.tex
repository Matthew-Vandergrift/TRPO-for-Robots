...
\begin{frame}{Cheap constrained optimization solvers}
    ...
    \only<2>{\begin{align*} 
        ...
        & \hat{D}_{KL} (\theta_{old} \vert\vert \theta) \approx 0 + 0 + \frac{1}{2}(\theta - \theta_{old})^TH(\theta - \theta_{old}) \\ 
        &  \hat{D}_{KL} (\theta_{old} \vert\vert \theta) \approx  \frac{1}{2}(\theta - \theta_{old})^TH(\theta - \theta_{old})
    \end{align*}}
\end{frame}
