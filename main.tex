...
\only<2>{\begin{align*} 
    ...
    \intertext{Let $(\theta - \theta_{old}) = \Delta \theta$}
...
\begin{frame}{Cheap constrained optimization solvers}
    ...
    \only<2>{\begin{align*} 
        & \hat{D}_{KL} (\theta_{old} \vert\vert \theta) \approx \hat{D}_{KL} (\theta_{old} \vert\vert \theta_{old})  + \nabla_{\theta} \hat{D}_{KL} (\theta_{old} \vert\vert \theta)\vert_{\theta_{old}} \cdot(\theta - \theta_{old}) + \\
        & \hspace{15pt} \frac{1}{2}(\theta - \theta_{old})^TH(\theta - \theta_{old}) \\
        & \hat{D}_{KL} (\theta_{old} \vert\vert \theta) \approx 0 + 0 + \frac{1}{2}\Delta\theta ^TH\Delta\theta \\ 
        &  \hat{D}_{KL} (\theta_{old} \vert\vert \theta) \approx  \frac{1}{2}\Delta\theta ^TH\Delta\theta 
    \end{align*}}
\end{frame}
...
\begin{frame}{Steps to Solve}
    ...
    \begin{align*}
        ...
        \only<2->{& L = g^T + \lambda \left( \frac{1}{2}(\theta - \theta_{old})^TH(\theta - \theta_{old}) - \delta \right)} \\ 
